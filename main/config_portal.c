...
/* Company logo embedded in the firmware and served as a small static asset. */
...
/* NTP host input is intentionally narrow: IPv4 or DNS hostname only. */
...
/* Wi-Fi is a live connection.  Saving a device ID or MQTT setting must not
 * reconfigure the STA interface, because doing so interrupts a healthy link. */
...
/* MQTT can be restarted independently of STA.  This deliberately includes the
 * logical ID because it is also the generated MQTT client ID and topic root. */
...
{
    /* A successful save is a Callbox application action, so acknowledge it
     * through the tower buzzer on DO1. GPIO46 is reserved for STA network
     * connect/disconnect notifications in network_status_task. */
    status_request_feedback(OUTPUT_FEEDBACK_CONFIG_SAVED);
}

/* Keep fallback/legacy markup and the active Vietnamese page on one AP URL.
...
/* The setup page is minified and gzip-compressed before being stored in flash. */
...
static bool request_from_local_ap(httpd_req_t *req)
{
    ...
    /* Fallback for stacks that report INADDR_ANY from getsockname(). */
    uint32_t peer_ip = 0;
...
static esp_err_t wifi_scan_handler(httpd_req_t *req)
{
    ...
    for (int attempt = 0; attempt < 12; attempt++) {
        ...
        /* The Wi-Fi manager is owner of association state. Never disconnect
         * the STA from an HTTP request merely to make a scan succeed. */
...
{
    if (!require_portal_access(req)) return ESP_OK;
    /* The preferred profile (s_config->wifi_ssid) is not necessarily the
     * network currently selected by the Wi-Fi driver: it may select a
     * stronger remembered AP.  Read the runtime STA SSID for the UI label. */
...
{
    if (!require_portal_access(req)) return ESP_OK;
    /* A finished setup session must stay closed. The page heartbeat may
     * still arrive for a few seconds while the browser is closing; it must
     * not reopen the AP session and keep the AP-status output active. */
