...
"const gw=(()=>{const fetchJson=async u=>{const r=await fetch(u,{cache:'no-store'});if(!r.ok)throw Error(await r.text());return r.json()};let modal,modalState,modalTail=Promise.resolve();function modalNode(){if(modal)return modal;modal=document.createElement('dialog');modal.className='gw-modal';modal.setAttribute('role','alertdialog');modal.setAttribute('aria-modal','true');modal.setAttribute('aria-labelledby','gwModalTitle');modal.setAttribute('aria-describedby','gwModalMessage');modal.innerHTML=`<div class='gw-modal-brand'><img src='/logo.png' alt='AUBOT'><span>WAREHOUSE GATEWAY</span></div><div class='gw-modal-content'><div class='gw-modal-icon' aria-hidden='true'><svg viewBox='0 0 24 24'><circle cx='12' cy='12' r='9'></circle><path d='M12 10v6M12 7h.01'></path></svg></div><div class='gw-modal-copy'><h2 id='gwModalTitle'>Thông báo</h2><p id='gwModalMessage'></p></div></div><div class='gw-modal-actions'><button class='btn secondary' type='button' data-cancel>Hủy</button><button class='btn primary' type='button' data-ok>Đã hiểu</button></div>`;document.body.append(modal);modal.querySelector('[data-cancel]').onclick=()=>settle(false);modal.querySelector('[data-ok]').onclick=()=>settle(true);modal.addEventListener('cancel',e=>{e.preventDefault();settle(false)});modal.addEventListener('click',e=>{if(e.target===modal)settle(false)});modal.addEventListener('keydown',e=>{if(e.key!=='Tab')return;const controls=Array.from(modal.querySelectorAll('button:not([hidden]):not(:disabled)'));if(!controls.length)return;const first=controls[0],last=controls[controls.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus()}});return modal}function settle(value){if(!modalState)return;const state=modalState;modalState=null;if(modal.open)modal.close();if(state.focus&&state.focus.isConnected)state.focus.focus();state.resolve(state.confirm?value:true)}function present(message,options,confirmMode){const run=()=>new Promise(resolve=>{const d=modalNode(),o=options||{},cancel=d.querySelector('[data-cancel]'),ok=d.querySelector('[data-ok]'),icon=d.querySelector('.gw-modal-icon');modalState={resolve,confirm:confirmMode,focus:document.activeElement};d.querySelector('#gwModalTitle').textContent=o.title||(confirmMode?'Xác nhận thao tác':'Thông báo');d.querySelector('#gwModalMessage').textContent=String(message==null?'':message);cancel.hidden=!confirmMode;cancel.textContent=o.cancelText||'Hủy';ok.textContent=o.confirmText||(confirmMode?'Đồng ý':'Đã hiểu');ok.className='btn '+(o.danger?'danger':'primary');icon.classList.toggle('danger',!!o.danger);d.showModal();((confirmMode&&o.danger)?cancel:ok).focus()});const result=modalTail.then(run,run);modalTail=result.then(()=>undefined,()=>undefined);return result}const alertDialog=(message,options)=>present(message,options,false),confirmDialog=(message,options)=>present(message,options,true);const page=document.body.dataset.page||'';const routes=[['overview','/','TỔNG QUAN'],['factory','/app/factory','KHO'],['tech','/app/tech','CHẨN ĐOÁN'],['it','/app/it','KẾT NỐI']];let me={authenticated:false,role:'PUBLIC'};function allowed(key){if(key==='overview')return true;if(!me.authenticated)return false;if(me.role==='AUBOT')return true;if(key==='factory')return me.role==='FACTORY'||me.role==='TECH';if(key==='tech')return me.role==='TECH';if(key==='it')return me.role==='FACTORY'||me.role==='TECH'||me.role==='IT';return false}function nav(){const n=document.querySelector('[data-app-nav]');if(!n)return;n.innerHTML=routes.map(x=>'<a class=\"'+(x[0]===page?'active':'')+'\" href=\"'+x[1]+'\" data-key=\"'+x[0]+'\">'+x[2]+'</a>').join('')+(me.authenticated?'<details class=\"user-menu\"><summary><span>'+me.username+'</span><small>'+me.role+'</small></summary><div><form method=\"post\" action=\"/logout\"><button class=\"btn ghost\">Đăng xuất</button></form></div></details>':'');n.querySelectorAll('[data-key]').forEach(a=>{const ok=allowed(a.dataset.key);if(me.authenticated&&!ok){a.setAttribute('aria-disabled','true');a.title='Tài khoản này không có quyền truy cập'}a.onclick=e=>{if(ok)return;e.preventDefault();if(!me.authenticated)location='/login?next='+encodeURIComponent(a.getAttribute('href'));else void alertDialog('Tài khoản này không có quyền vào trang đã chọn.',{title:'Không có quyền truy cập'})}})}function renderSystem(s){document.querySelectorAll('[data-system-bar]').forEach(bar=>{bar.className='system-bar';const canState=s.can==='ACTIVE'?'ok':s.can==='WARNING'||s.can==='PASSIVE'?'warn':'bad',ethIp=s.eth_ip||'--',values=[['Gateway',s.gateway_id,'ok'],['WiFi STA',s.sta?(s.sta_ip||'ĐÃ KẾT NỐI'):'OFFLINE',s.sta?'ok':'bad'],['AP cấu hình',(s.ap?'BẬT':'TẮT')+' · '+(s.ap_ip||'--'),s.ap?'ok':'warn'],['Ethernet '+(s.eth_mode==='UPLINK'?'UPLINK':'DEBUG'),!s.eth_link?'CHƯA CẮM CÁP · '+ethIp:s.ethernet?ethIp:'ĐANG LẤY IP · '+ethIp,s.eth_link&&s.ethernet?'ok':s.eth_link?'warn':'bad'],['MQTT',s.mqtt?'ONLINE':'OFFLINE',s.mqtt?'ok':'bad'],['CAN',s.can,canState]];bar.innerHTML=values.map(x=>'<div class=\"system-item '+x[2]+'\" title=\"'+x[0]+': '+x[1]+'\"><i></i><span><small>'+x[0]+'</small><b>'+x[1]+'</b></span></div>').join('')})}async function system(){if(document.hidden){setTimeout(system,10000);return}try{const s=await fetchJson('/api/ui/status');sessionStorage.setItem('gwStatus',JSON.stringify(s));renderSystem(s)}catch(e){}setTimeout(system,4000)}try{const s=JSON.parse(sessionStorage.getItem('gwStatus'));if(s)renderSystem(s)}catch(e){}nav();fetchJson('/api/auth/me').then(x=>{me=x;const u=document.querySelector('[data-app-user]');if(u&&me.authenticated)u.innerHTML='<div class=\"user-box\"><span class=\"dot ok\"></span><span><small>'+me.role+'</small><b>'+me.username+'</b></span></div>';nav()}).catch(()=>{});system();return{fetchJson,alert:alertDialog,confirm:confirmDialog}})();";
...
static esp_err_t ui_status_handler(httpd_req_t *request)
{
    ...
    const bool ap_active = platform_wifi_ap_is_active();
    const bool eth_debug = !config.eth_router_mode;
    const char *eth_ip = ethernet.ip[0] != '\0' ? ethernet.ip
                                                  : (eth_debug ? GATEWAY_ETH_DEBUG_IP : "");
    char json[512];
    const int length = snprintf(json, sizeof(json),
        "{\"gateway_id\":\"%s\",\"network\":%s,\"mqtt\":%s,\"can\":\"%s\","
        "\"sta\":%s,\"sta_ip\":\"%s\",\"ap\":%s,\"ap_ip\":\"%s\","
        "\"ethernet\":%s,\"eth_link\":%s,\"eth_ip\":\"%s\",\"eth_mode\":\"%s\"}",
        ...
        wifi.connected ? "true" : "false", wifi.ip,
        ap_active ? "true" : "false", GATEWAY_AP_IP,
        ethernet.connected ? "true" : "false",
        bsp_eth_link_is_up() ? "true" : "false", eth_ip,
        eth_debug ? "DEBUG" : "UPLINK");
