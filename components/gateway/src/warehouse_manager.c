...
static esp_err_t persist_all(void)
{
    ...
    for (uint8_t i = 0; e == ESP_OK && i < WAREHOUSE_POSITION_MAX; ++i) {
        char key[8], value[104]; key_for_position(i + 1U, key);
        const warehouse_position_config_t *p = &positions[i];
        int n = snprintf(value, sizeof(value), "%u|%u|%u|%u|%u|%u|%u|%u|%s|%s",
...
static bool decode(uint8_t position_id, char *value, warehouse_position_config_t *p)
{
    ...
    if (count != 8U && count != 9U && count != 10U) return false;
    ...
    const size_t text_index = separated ? 8U : count == 9U ? 7U : 6U;
    p->proximity_enabled = separated || count == 9U
        ? strtoul(part[text_index - 1U], NULL, 10) != 0 : p->enabled;
...
esp_err_t warehouse_manager_init(void)
{
    ...
    for(uint8_t position=1;position<=WAREHOUSE_POSITION_MAX;position++){char key[8],v[104]={0};key_for_position(position,key);found=false;e=platform_nvs_get_string(&h,key,v,sizeof(v),&found);if(e!=ESP_OK)break;if(found&&v[0]){warehouse_position_config_t p;if(decode(position,v,&p))s_positions[position-1U]=p;}}
    platform_nvs_close(&h);
...
warehouse_validation_t warehouse_manager_validate_profile(laser_profile_t profile)
{
    ...
    taskENTER_CRITICAL(&s_mux);
    for(uint8_t i=0;i<WAREHOUSE_POSITION_MAX;i++){const warehouse_position_config_t *p=&s_positions[i];if(p->enabled&&(p->position_id>count||p->group_id==0||p->group_id>count||!laser_profile_id_allowed(profile,p->group_id,p->laser_id))){taskEXIT_CRITICAL(&s_mux);return WAREHOUSE_PROFILE_CONFLICT;}}
...
esp_err_t warehouse_manager_set_position(const warehouse_position_config_t *p)
{
    ...
    taskENTER_CRITICAL(&s_mux);s_positions[p->position_id-1U]=*p;taskEXIT_CRITICAL(&s_mux);return persist_all();
}
